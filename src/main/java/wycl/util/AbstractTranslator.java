 ...
 *
 */
public abstract class AbstractTranslator<D, S, E, T> {
	...
	public S visitInitialiser(Stmt.Initialiser stmt) {
		T type = visitType(stmt.getType());
		E initialiser = null;
		if (stmt.hasInitialiser()) {
			initialiser = visitExpression(stmt.getInitialiser());
		}
		return constructInitialiser(stmt, type, initialiser);
	}
	...
	public T visitType(Type type) {
		...
		case TYPE_array:
			return visitArrayType((Type.Array)type);
		case TYPE_bool:
			return visitBoolType((Type.Bool)type);
		case TYPE_byte:
		...
		case TYPE_reference:
			return visitReferenceType((Type.Reference)type);
		case TYPE_union:
			return visitUnionType((Type.Union)type);
		case TYPE_void:
			return visitVoidType((Type.Void)type);
		default:
			throw new IllegalArgumentException("unknown expression encountered (" + type.getClass().getName() + ")");
		}
	}

	public T visitArrayType(Type.Array type) {
		T element = visitType(type.getElement());
		return constructArrayType(type,element);
	}

	public T visitBoolType(Type.Bool type) {
		return constructBoolType(type);
	}

	public T visitByteType(Type.Byte type) {
		return constructByteType(type);
	}

	public T visitIntType(Type.Int type) {
	...
	public T visitReferenceType(Type.Reference type) {
		T element = visitType(type.getElement());
		return constructReferenceType(type,element);
	}

	public T visitUnionType(Type.Union type) {
		ArrayList<T> bounds = new ArrayList<>();
		for (int i = 0; i != type.size(); ++i) {
			bounds.add(visitType(type.get(i)));
		}
		return constructUnionType(type, bounds);
	}

	public T visitVoidType(Type.Void type) {
		return constructVoidType(type);
	}
	...
	public abstract S constructInitialiser(Stmt.Initialiser stmt, T type, E initialiser);
	...
	public abstract T constructArrayType(Type.Array type, T element);

	public abstract T constructBoolType(Type.Bool type);

	public abstract T constructByteType(Type.Byte type);
	...
	public abstract T constructUnionType(Type.Union type, List<T> element);

	public abstract T constructVoidType(Type.Void type);
